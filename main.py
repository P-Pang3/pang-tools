...
class MacroApp:
    ...
    def _build_run_tab(self):
        ...
        tk.Label(
            self.help_frame,
            text=(
                "   [템플릿 스캔 모드]  템플릿 탭에 등록해둔 이미지를 찾아서 클릭\n"
                "   [고정 위치 모드]  템플릿이 없을 때 — 마우스 커서 자리에 그대로 클릭\n\n"
                "   1.  대상 프로그램을 켜고 → 템플릿 탭에서 [새 템플릿 캡처] 클릭\n"
                "   2.  창이 내려가면 화면에서 찾을 것을 드래그로 선택\n"
                "   3.  Home (또는 [시작]) 으로 시작\n"
                "   4.  멈출 땐 Home 다시 또는 F12 (비상 정지)\n"
                "        단축키는 설정 탭에서 바꿀 수 있습니다\n"
